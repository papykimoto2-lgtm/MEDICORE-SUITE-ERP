...
const MEDICORE_DEMO = {
  ...
  injecter(force){
    ...
    // IMPORTANT : le store 'prescriptions' est un TABLEAU plat géré par MEDICORE_RX.
    // On n'écrit JAMAIS un objet dedans (sinon .filter casse). On passe par RX.ajouter.
  ...
  supprimer(){
    ...
    // Prescriptions démo : retirer les RXDEMO* du tableau plat MEDICORE_RX
    this._save('prescriptions', this._load('prescriptions').filter(r=>!(r && String(r.id).startsWith('RXDEMO'))));
    // Stores objet liés au parcours démo → vidés (objets, pas tableaux)
    ['bioResultats','constantes','timeline'].forEach(s=>this._saveObj(s, {}));
    // Stores tableau liés au parcours démo → vidés
    ['prestations','resultats','demandes'].forEach(s=>this._save(s, []));
