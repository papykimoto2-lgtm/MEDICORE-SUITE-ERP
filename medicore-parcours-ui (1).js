...
(function(){
  ...
  function render(){
    ...
    if(modaleOuverte){ if(!bar.classList.contains('hidden')) bar.classList.add('hidden'); return; }
  ...
  function init(){
    ...
    // ── Détection ouverture/fermeture modale, SANS boucle de reflow ──
    // On n'observe PAS tout le subtree (trop coûteux). On surveille seulement
    // l'attribut class du body et des overlays connus, et on debounce.
    try{
      const obs=new MutationObserver(()=>{
        // Ignorer les mutations provoquées par notre propre barre
        renderDebounced();
      });
      // Surveiller uniquement les overlays existants (pas tout le document)
      document.querySelectorAll('.overlay, .modal-overlay').forEach(ov=>{
        obs.observe(ov, { attributes:true, attributeFilter:['class','style'] });
      });
      // + le body pour les overlays créés dynamiquement (ajout/retrait d'enfants)
      const bodyObs=new MutationObserver((muts)=>{
        const pertinent = muts.some(m=>
          [...m.addedNodes, ...m.removedNodes].some(n=>n.nodeType===1 && (n.classList&&(n.classList.contains('overlay')||n.classList.contains('modal-overlay'))))
        );
