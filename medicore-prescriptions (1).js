...
const MEDICORE_RX = {
  ...
  _read(){
    let data;
    if(typeof MEDICORE_STORE!=='undefined') data = MEDICORE_STORE.load(this.STORE, []);
    else { try{ const p=JSON.parse(localStorage.getItem('medicore_'+this.STORE)||'[]'); data = Array.isArray(p)?p:(p.d||p); }catch(e){ return []; } }
    // Robustesse : si une autre source a écrit un objet {patientId:[...]} au lieu d'un tableau,
    // on l'aplatit en tableau pour éviter ._read().filter is not a function.
    if(Array.isArray(data)) return data;
    if(data && typeof data==='object'){
      const flat=[];
      Object.keys(data).forEach(k=>{ if(Array.isArray(data[k])) data[k].forEach(r=>flat.push(Object.assign({patientId:k}, r))); });
      this._write(flat);  // ré-écrit proprement
      return flat;
    }
    return [];
  },
